...
void main()
{
	...
	vec4 t0 = texture2D(inputImageTexture3, vec2(textureCoordinate.x, textureCoordinate.y));

	// naver skin
//    vec4 c2 = vec4(0.0,0.0,0.0,1.0);
	vec4 c2 = texture2D(inputImageTexture, textureCoordinate);

	c2 = normal(c2, screen  (c2, c2), 0.275); // screen 70./255.
	c2 = normal(c2, overlay (c2, vec4(1., 0.61176, 0.25098, 1.)), 0.04); // overlay 10./255.

	c2 = normal(c2, multiply(c2, t0), 0.262); // multiply 67./255.
	c2 = level0c(c2,inputImageTexture2);
