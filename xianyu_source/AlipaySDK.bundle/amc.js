...
if (amc.isH5) {
    // rpcData
    window.flybird = window.MQPJSBridge.startupParams;

    if (window.MQPJSBridge && window.MQPJSBridge.startupParams.local) {
        var u = navigator.userAgent;
        var isIOS = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
        MQPJSBridge.startupParams.local.platform = isIOS ? 'iOS' : 'android';
    }

    // 启动参数
    document.getCustomAttr = function(name) {
        try {
            return window.MQPJSBridge.startupParams.local[name];
        } catch (e) {
            return null;
        }
    };
