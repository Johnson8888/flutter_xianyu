(function() {
	...
	var callback = function(records) {
		...
		for (var key in records) {
			...
			// 过滤删除操作之后的改变（有删除操作即可认为当前节点位置已经完成首屏）
			...
			if (show == undefined) {
				...
				// 1 节点自身是否可见
				var show1 = domNodeTop < visibleBottom && domNodeBottom > visibleTop && domNodeLeft < visibleRight && domNodeRight > visibleLeft;
				// 2 过滤不完全显示的节点
				var show2 = true;
				if (visibleBottom < domNodeBottom && visibleBottom - domNodeTop < 30) {
					show2 = false;
				}
				...
				if (record.addedNodes.length > 0) {
					...
					if (firstAddedDom != undefined && firstAddedDom.nodeType == 1) {
						...
						// 3 判断子节点是否可见
						show3 = addedDomNodeTop < visibleBottom && addedDomNodeBottom > visibleTop && addedDomNodeLeft < visibleRight && addedDomNodeRight > visibleLeft;
						// 4 过滤不完全显示的子节点
						if (visibleBottom < addedDomNodeBottom && visibleBottom - addedDomNodeTop < 30) {
							show4 = false;
						}
					}
				}
			...
			if (show && filter) {
				...
				if (badCaseMetaName == undefined) {
					...
					if (currentTime - previousTime > 10) {
						...
						var domNodeDetail = new Object();
						var addedDomNodeDetail = new Object();
						message.time = currentTime;
						message.domNodeDetail = domNodeDetail;
						message.addedDomNodeDetail = addedDomNodeDetail;
						domNodeDetail.tagName = domNode.tagName;
						domNodeDetail.className = domNode.className;
						domNodeDetail.id = domNode.id;
						if(record.addedNodes.length > 0) {
							var firstAddedDom = record.addedNodes[0];
							if(firstAddedDom != undefined) {
								addedDomNodeDetail.tagName = firstAddedDom.tagName;
								addedDomNodeDetail.className = firstAddedDom.className;
								addedDomNodeDetail.id = firstAddedDom.id;
							}
						}
 
						window.webkit.messageHandlers.WindVaneFirstScreenPaintTime.postMessage(JSON.stringify(message));
